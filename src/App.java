 ...
 * - If max is greater than 0, compute what percent n is of max as a DECIMAL
 * calculation.
...
public class App {
    public static void main(String[] args) {
        ...
        System.out.println("Give another posative number");
        int max = Integer.valueOf(scanner.nextLine());
        if (0 >= max || max >= 100) {
            System.out.println("Wrong number!");
        } else {
            System.out.println(((number * 1.0) / max) * 100 + "%");
            if ((((number * 1.0) / max) * 100) >= 45 && (((number * 1.0) / max) * 100) < 50
                    || (((number * 1.0) / max) * 100) > 50 && (((number * 1.0) / max) * 100) <= 55) {
                System.out.println("Almost halfway");
            } else {
                if ((((number * 1.0) / max) * 100) == 50) {
                    System.out.println("Halfway!");
                } else {
                    if ((((number * 1.0) / max) * 100) > 55) {
                        System.out.println("Above Halfway");
                    } else {
                        if ((((number * 1.0) / max) * 100) < 45) {
                            System.out.println("Below halfway");
                        }
                    }
                }
            }
